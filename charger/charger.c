...
static void process_ps_uevent(struct charger *charger, struct uevent *uevent)
{
    ...
    if (!strncmp(uevent->ps_name, BATTERY_DEVICE_NAME, 2))
        battery = true;
    ...
    if (!(strcmp(AC_DEVICE_NAME, uevent->ps_name)) || !(strcmp(USB_DEVICE_NAME, uevent->ps_name))) {
        if (was_online && !online)
	  charger->num_supplies_online=0; //TODO put this to zero
        else if (supply && !was_online && online)
            charger->num_supplies_online=1;
    }
